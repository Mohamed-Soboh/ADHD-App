...
function bookCalendar() {
	...
	$bkdate		= $rdate. ' '. $rtime;
	$ucount		= $_POST['ucount'];
	$ids= $_SESSION['calendar_fd_user']['id'];
	...
	*/
	...
	$sql = "INSERT INTO tbl_reservations (uid, ucount, rdate, status, comments, bdate) 
			VALUES ('$title', $ids, '$bkdate', 'PENDING', '', NOW())";
	dbQuery($sql);
	...
	//send_email($data);
	header('Location: ../index.php?msg=' . urlencode('You Add Task successfully .'));//dsssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssdsdsdsdsdssdsd
	exit();
}
...
function regConfirm() {
	...
	header('Location: ../views/?v=DB&msg=' . urlencode('task status successfully changed.'));
	exit();
}
...
$sql	= "SELECT r.uid AS u_name, u.id AS user_id, r.rdate, r.status 
...
if ($type=='parent') {
	$id=$_SESSION['calendar_fd_user']['sonid'];
	$sql	= "SELECT r.uid AS u_name, u.id AS user_id, r.rdate, r.status 
			   FROM tbl_users u, tbl_reservations r 
			   WHERE u.id = r.ucount AND r.ucount= $id
			   AND (r.rdate BETWEEN '$start' AND '$end')";
	...
	while($row = dbFetchAssoc($result)) {
		...
		$t=time();
		if($type=='student'){
		$book->url = "http://10.0.0.1/event-management/event-management/feedback/";
